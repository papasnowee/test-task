...
export const calcPassedTime = (time: Date) => {
    ...
    if (hours >= 1 && hours < 24) {
        ...
        if (hours === 2 || hours === 3 || hours === 4 || hours === 22 || hours === 23) return `${hours} часа назад`
...
export function timeAgo(date: Date) {
    ...
    if (timeElapsed < minute) {
        ...
        return `${minutesAgo} ${declOfNum(minutesAgo, ['минуту', 'минуты', 'минут'])} назад`
    } else if (timeElapsed < day) {
        const hoursAgo = Math.round(timeElapsed / hour)
        return `${hoursAgo} ${declOfNum(hoursAgo, ['час', 'часа', 'часов'])} назад`
    } else {
...
export function createObjectList<P>(objectList: P[]) {
    ...
    objectList.forEach((item) => commentsWithChildren.push(Object.assign({}, item)))
